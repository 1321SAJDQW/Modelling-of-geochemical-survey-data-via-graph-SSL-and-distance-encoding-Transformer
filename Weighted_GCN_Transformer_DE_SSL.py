...
class TransformerModule(nn.Module):
    def __init__(self, emb_size, num_heads=8, num_layers=4, dropout=0.1, edge_weights=None, lambda_param=1):
    ...
    def forward(self, x):
        if self.edge_weights is None:
            raise ValueError("Edge weights must be provided during initialization.")

        edge_weights = self.edge_weights
        edge_weights = torch.tensor(edge_weights).to(x.device)
        edge_weights = edge_weights.to_dense()

        shortest_path = np.load('shortest_paths_matrix.npy')
        shortest_path = torch.tensor(shortest_path).to(x.device)

        shortest_path_inv = 1 / (shortest_path)
        shortest_path_inv[shortest_path == 0] = 0
...
Transformer = TransformerModule(emb_size=args.emb_size, edge_weights=adj_ori).to(device)
