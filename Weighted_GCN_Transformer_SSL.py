...
parser = argparse.ArgumentParser(description="Weighted GCN-Transformer SSL for Mineral Prospectivity Mapping")
...
# ----------------------------------------------------------------------------------------------------------------------
# Load Data and Extract Weighted Graph Topology
# ----------------------------------------------------------------------------------------------------------------------
...
for epoch in range(args.epochs):
    ...
    emb_before_final = Transformer(emb_before_gcn.unsqueeze(0)).squeeze(0)

    emb_after_gcn = GCN(masked_geo_features, edge_index, edge_weight)
    emb_after_final = Transformer(emb_after_gcn.unsqueeze(0)).squeeze(0)
...
with torch.no_grad():
    ...
    emb = Transformer(emb_gcn.unsqueeze(0)).squeeze(0)
...
data.to_csv('Weighted_GCN_Transformer_SSL.csv', index=False)
...
full_results_df.to_csv('GCN_Transformer_SSL.csv', index=False)
pd.DataFrame({'Metric': list(metrics.keys()), 'Score': list(metrics.values())}).to_csv(
    'Metric_Weighted_GCN_Transformer_SSL.csv', index=False)